...
(function (root) {
  ...
  /**
   * 抽取页面上「所有能直接提取的文字」，用于语言判断。
   *
   * 与旧版的关键差异：旧版只取正文前 4000 字符（collectSampleText），
   * 一旦页面头部 / 语言地区选择器里混了别国文字，整页就会被误判
   * （例如 NVIDIA 中文站的地区选择器里有韩文，导致整页被判成韩语）。
   * 现在改成遍历全页、统计全部可提取文字，只用一个很高的兜底上限防止极端页面卡顿。
   *
   * @param {Document} doc
   * @param {{limit?:number, maxNodes?:number}} [opts]
   * @returns {{text:string, scripts:Object, chars:number, nodes:number, truncated:boolean}}
   */
  function collectPageText(doc, opts) {
    doc = doc || (typeof document !== 'undefined' ? document : null);
    opts = opts || {};
    var limit = opts.limit || 200000;      // 兜底上限：正常页面远达不到
    var maxNodes = opts.maxNodes || 40000;
    var empty = { text: '', scripts: { han: 0, kana: 0, hangul: 0 }, chars: 0, nodes: 0, truncated: false };
    ...
    var visited = 0;
    var truncated = false;
    var scripts = { han: 0, kana: 0, hangul: 0, cyrillic: 0, arabic: 0, hebrew: 0,
      thai: 0, devanagari: 0, bengali: 0, greek: 0, latin: 0 };

    // 不参与判断的标签（连同子树跳过）。
    // 注意：SELECT / OPTION 不在此列 —— 下拉框里选中的语言名用户是看得见的
    //（很多站点的页头就是「한국어 / 简体中文 / English」这样的选择器），
    // 把它们算进统计反而更贴近用户实际看到的页面。
    var SKIP = {
      SCRIPT: 1, STYLE: 1, NOSCRIPT: 1, TEMPLATE: 1, IFRAME: 1, CANVAS: 1,
      SVG: 1, MATH: 1, TEXTAREA: 1, INPUT: 1
    };

    function walk(el) {
      if (!el || visited > maxNodes || truncated) return;
      ...
      while (child) {
        ...
        if (child.nodeType === 3) {
          ...
          if (t && t.trim()) {
            countScriptsInto(t.trim(), scripts);
            if (total < limit) {
              parts.push(t.trim());
              total += t.trim().length;
            } else {
          ...
          var hidden = false;
          try {
            if (child.hasAttribute && child.hasAttribute('hidden')) hidden = true;
            if (child.getAttribute && child.getAttribute('aria-hidden') === 'true') hidden = true;
            var cs = doc.defaultView ? doc.defaultView.getComputedStyle(child) : null;
            if (cs && (cs.display === 'none' || cs.visibility === 'hidden')) hidden = true;
          } catch (e) { /* ignore */ }
          if (!hidden) walk(child);
        }
        child = next;
      }
    }
    ...
    walk(doc.body);
    return { text: parts.join('\n'), scripts: scripts, chars: total, nodes: visited, truncated: truncated };
  }

  /** 把一段文字的字符计入 script 统计表 */
  function countScriptsInto(text, counts) {
    for (var i = 0; i < text.length; i++) {
      var ch = text[i];
      for (var j = 0; j < SCRIPT_RANGES.length; j++) {
        if (SCRIPT_RANGES[j][1].test(ch)) {
          counts[SCRIPT_RANGES[j][0]] = (counts[SCRIPT_RANGES[j][0]] || 0) + 1;
          break;
        }
      }
    }
  ...
  /**
   * 由文字系统统计判断主导文字。
   * 关键改动：不再用「某个文字出现 4 次就成立」这种阈值，
   * 而是比较汉字 / 假名 / 谚文的相对占比，避免混排页面被误判。
   */
  function dominantScript(counts) {
    counts = counts || {};
    var han = counts.han || 0, kana = counts.kana || 0, hangul = counts.hangul || 0;
    var cjk = han + kana + hangul;

    // 日文：假名是它独有的，只要占比可观就成立
    if (kana > 0 && (kana >= 4 || kana * 10 >= han)) {
      return { code: 'ja', script: 'kana', confidence: cjk > 0 ? 0.6 + Math.min(0.35, kana / cjk) : 0.8 };
    }
    // 韩文：谚文与汉字混排时，必须谚文明显占优才算韩语
    //（韩文里汉字占比通常很低；中文页面里偶发的朝鲜语选项不该把整页判成韩语）
    if (hangul >= 4 && (hangul * 2 >= han || han < 10)) {
      return { code: 'ko', script: 'hangul', confidence: cjk > 0 ? 0.6 + Math.min(0.35, hangul / cjk) : 0.8 };
    }
    // 中文：汉字占主导（即便混了一点谚文 / 假名也仍然算中文）
    if (han >= 4) {
      return { code: 'zh', script: 'han', confidence: cjk > 0 ? 0.6 + Math.min(0.39, han / cjk) : 0.8 };
    }

    // 其余文字系统：取出现最多的那个
    var order = [
      ['cyrillic', 'ru'], ['arabic', 'ar'], ['hebrew', 'he'], ['thai', 'th'],
      ['devanagari', 'hi'], ['bengali', 'bn'], ['greek', 'el'], ['latin', 'en']
    ];
    var best = null;
    for (var i = 0; i < order.length; i++) {
      var n = counts[order[i][0]] || 0;
      if (n >= 4 && (!best || n > best.n)) best = { n: n, code: order[i][1], script: order[i][0] };
    }
    return best || null;
  }

  /**
   * 网页声明（<html lang> / meta content-language / og:locale）与整页统计是否冲突。
   * 只有「声明了某语言，且页面统计确实支持这个声明」才视为不冲突。
   */
  function declarationConflicts(det) {
    if (!det || !det.declared) return false;
    var d = det.declared;
    if (d === 'zh_Hans' || d === 'zh_Hant') return (det.hanCount || 0) < 10;
    if (d === 'ja') return (det.kanaCount || 0) < 4 && (det.hanCount || 0) < 10;
    if (d === 'ko') return (det.hangulCount || 0) < 2;   // 声明韩语时谚文极少的页面不可信
    var scriptOfDeclared = {
      en: 'latin', fr: 'latin', de: 'latin', es: 'latin', pt: 'latin', it: 'latin',
      nl: 'latin', vi: 'latin', id: 'latin', tr: 'latin', pl: 'latin', ro: 'latin',
      sv: 'latin', da: 'latin', cs: 'latin', hu: 'latin', fi: 'latin',
      ru: 'cyrillic', ar: 'arabic', he: 'hebrew', th: 'thai', hi: 'devanagari', el: 'greek'
    }[d];
    if (scriptOfDeclared && det.script) return scriptOfDeclared !== det.script;
    return false;
  }
   ...
   * @returns {object} detectText 的返回值 + declared / 采样统计等附加字段
   */
  function detectDocument(doc, opts) {
    doc = doc || (typeof document !== 'undefined' ? document : null);
    ...
    var sample = collectPageText(doc, opts);
    var res = detectText(sample.text, declared);

    // 最终裁决交给整页统计（比 detectText 内部的局部打分可靠）
    var dom = dominantScript(sample.scripts);
    if (dom) {
      res.script = dom.script;
      if (dom.code !== 'zh') {
        res.code = dom.code;
        res.name = LANG_NAME[dom.code] || SCRIPT_NAME[dom.script] || res.name;
        res.isChinese = false;
        res.isSimplifiedChinese = false;
      } else {
        // 中文：再用简繁特征字区分
        var cjkOnly = sample.text.replace(/[^\u4E00-\u9FFF\u3400-\u4DBF]/g, '');
        if (traditionalScore(cjkOnly) > 0) {
          res.code = 'zh_Hant';
          res.name = LANG_NAME.zh_Hant;
          res.isChinese = true;
          res.isSimplifiedChinese = false;
        } else {
          res.code = 'zh_Hans';
          res.name = LANG_NAME.zh_Hans;
          res.isChinese = true;
          res.isSimplifiedChinese = true;
        }
      }
      if (typeof dom.confidence === 'number') {
        res.confidence = Math.max(res.confidence, Math.min(0.98, dom.confidence));
      }
    }

    res.declared = declared ? normalizeLangCode(declared) : '';
    res.declaredRaw = declared || '';
    res.scriptCounts = sample.scripts;
    res.sampleChars = sample.chars;
    res.sampleNodes = sample.nodes;
    res.sampleTruncated = sample.truncated;
    res.hanCount = sample.scripts.han || 0;
    res.hangulCount = sample.scripts.hangul || 0;
    res.kanaCount = sample.scripts.kana || 0;
    res.declarationAgrees = !declarationConflicts(res);
    return res;
  }

  /** 网页声明与整页统计一致时，生成一个高置信度的结果 */
  function declaredResult(det) {
    var code = det.declared;
    return {
      code: code,
      lang: code,
      ...
      confidence: 0.9,
      mixed: false,
      isChinese: code === 'zh_Hans' || code === 'zh_Hant',
      isSimplifiedChinese: code === 'zh_Hans',
      declared: code,
      declaredRaw: det.declaredRaw || '',
      fromDeclaration: true,
      scriptCounts: det.scriptCounts,
      sampleChars: det.sampleChars,
      hanCount: det.hanCount,
      hangulCount: det.hangulCount,
      kanaCount: det.kanaCount,
      declarationAgrees: true
    };
  }
  ...
  function shouldTranslate(det) {
    if (!det) return false;
    // 网页声明与整页统计一致时以声明为准：声明简体中文就不翻译
    if (det.declared && !declarationConflicts(det)) {
      return det.declared !== 'zh_Hans';
    }
    // 日文 / 韩文里虽然含汉字，但仍是外语
  ...
  /** 兼容旧调用：仍取一段文本样本（内部已改为遍历全页，limit 只影响返回长度） */
  function collectSampleText(doc, limit) {
    var r = collectPageText(doc, { limit: Math.max(limit || 4000, 4000) });
    return r.text.slice(0, (limit || 4000) * 2);
  }
  ...
  var api = {
    ...
    countScriptsInto: countScriptsInto,
    ...
    declarationConflicts: declarationConflicts,
